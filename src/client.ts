...
import { Auth, AuthLoginParams, AuthLoginResponse, AuthRefreshParams, AuthRefreshResponse, AuthRegisterParams, AuthRegisterResponse, AuthResendOtpParams, AuthResetPasswordParams, AuthUpdatePasswordParams, AuthUpdatePasswordResponse, AuthVerifyOtpParams, AuthVerifyOtpResponse } from './resources/auth';
import { StatConversationsDailyResponse, StatOverviewResponse, StatUsageResponse, Stats } from './resources/stats';
import { Account } from './resources/account/account';
import { ProjectCreateParams, ProjectCreateResponse, ProjectExportProfilesResponse, ProjectListResponse, ProjectRetrieveResponse, ProjectUpdateParams, ProjectUpdateResponse, Projects } from './resources/projects/projects';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['DEEPRAVEN_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['DEEP_RAVEN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DEEP_RAVEN_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Deep Raven API. 
 */
export class DeepRaven {
  apiKey: string;
   ...
   * API Client for interfacing with the Deep Raven API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['DEEPRAVEN_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['DEEP_RAVEN_BASE_URL'] ?? https://deepraven.ai] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEEP_RAVEN_BASE_URL'),
    apiKey = readEnv('DEEPRAVEN_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.DeepRavenError(
        'The DEEPRAVEN_API_KEY environment variable is missing or empty; either provide it, or instantiate the DeepRaven client with an apiKey option, like new DeepRaven({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://deepraven.ai`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? DeepRaven.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('DEEP_RAVEN_LOG'), 'process.env[\'DEEP_RAVEN_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://deepraven.ai';
  }
  ...
  static DeepRaven = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static DeepRavenError = Errors.DeepRavenError;
...
export declare namespace DeepRaven {
