...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from 'deepraven'`:
   * - `import 'deepraven/shims/node'` (if you're running on Node)
   * - `import 'deepraven/shims/web'` (otherwise)
   */
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new DeepRavenError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new DeepRavenError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null) throw new DeepRavenError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new DeepRavenError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`DeepRaven:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new DeepRavenError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
