...
import { APIPromise } from 'deepraven/core/api-promise';

import util from 'node:util';
import DeepRaven from 'deepraven';
import { APIUserAbortError } from 'deepraven';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['DEEP_RAVEN_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: DeepRaven) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new DeepRaven({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['DEEP_RAVEN_LOG'] = 'debug';
    const client = new DeepRaven({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['DEEP_RAVEN_LOG'] = 'not a log level';
    const client = new DeepRaven({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'DEEP_RAVEN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new DeepRaven({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['DEEP_RAVEN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new DeepRaven({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['DEEP_RAVEN_BASE_URL'] = 'https://example.com/from_env';
      const client = new DeepRaven({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['DEEP_RAVEN_BASE_URL'] = ''; // empty
      const client = new DeepRaven({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://deepraven.ai')
    });

    test('blank env variable', () => {
      process.env['DEEP_RAVEN_BASE_URL'] = '  '; // blank
      const client = new DeepRaven({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://deepraven.ai')
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new DeepRaven({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['DEEP_RAVEN_BASE_URL'] = 'http://localhost:5000/env';
      const client = new DeepRaven({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new DeepRaven({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new DeepRaven({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['DEEPRAVEN_API_KEY'] = 'My API Key';
    const client = new DeepRaven();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['DEEPRAVEN_API_KEY'] = 'another My API Key';
    const client = new DeepRaven({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new DeepRaven({ apiKey: 'My API Key', fetch: testFetch });
